...
<a href="<?php echo U('Admin/index');?>" class="sidebar-icon"> <span class="fa fa-bars"></span> </a>
			...
			<div style="border-top:1px ridge rgba(255, 255, 255, 0.15)"></div>
                     <div class="menu">
						...
						<li id="menu-academico" ><a href="<?php echo U('Admin/rank');?>"><i class="fa fa-bar-chart"></i><span>排行榜</span> <span class="fa fa-angle-right" style="float: right"></span><div class="clearfix"></div></a>
							<ul>
...
function abook(obj,id){
    ...
    <script type="text/javascript">
		let cdata=[
		<?php
 for($j=0;$j<10;$j++){ echo '"' , $list[$j]['bookname'] , '"' ,','; } ?>
		];

		let bdata=[
		<?php
 for($i=0;$i<10;$i++){ echo $list[$i]['num'],','; } ?>
		];


		let digestb=[
		<?php
 for($j=0;$j<10;$j++){ echo '"' , $digest[$j]['bookname'] , '"' ,','; } ?>
		];

		let digestn=[
		<?php
 for($i=0;$i<10;$i++){ echo $digest[$i]['num'],','; } ?>
		];
